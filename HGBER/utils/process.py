...
def load_mat(dataset="Yelp"):
    """Load citation network dataset (cora only for now)"""
    print('Loading {} dataset...'.format(dataset))
    if dataset == 'Yelp':
        path = "./data/yelp2614.mat"
    elif dataset == 'ACM':
        path = "./data/ACM3025.mat"
    elif dataset == 'DBLP':
    ...
    if dataset == 'Yelp':
        for adj_name in ['BUB', 'BTB', 'BSB', 'BRB']:
            adjs.append(coo_matrix(data[adj_name]))
        features = data['features']
    if dataset == 'ACM':
        for adj_name in ['PAP', 'PLP']:
            adjs.append(coo_matrix(data[adj_name]))
    ...
    if dataset == 'DBLP':
        ...
        features = data['features']
    if dataset == 'IMDB':
        for adj_name in ['MAM', 'MDM']:
            adjs.append(coo_matrix(data[adj_name]))
        features = data['feature']
        label_index = data['i_label'][0] - 1
    features = coo_matrix(features)
    features = sp.csr_matrix(features, dtype=np.float32)
    if dataset != 'IMDB':
        label = [list(one_label).index(1) for one_label in list(data['label'])]
    else:
        label = [list(one_label).index(1) for one_label in list(data['label'][label_index])]
    labels = encode_onehot(label)

    original = range(features.shape[0])
