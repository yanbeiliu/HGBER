...
def evaluateKMeans(data, labels, settings):
    '''
    ...
    '''
    acc, nmi, ari = [], [], []
    nclusters = settings['num_class']
    for i in range(20):
        kmeans = KMeans(n_clusters=nclusters, n_init=1)
        kmeans.fit(data)
        pre_acc, pre_nmi, pre_ari = getClusterMetricString(labels, kmeans.labels_)
        acc.append(pre_acc)
        nmi.append(pre_nmi)
        ari.append(pre_ari)
    return np.average(acc), np.std(acc), np.average(nmi), np.std(nmi), np.average(ari), np.std(ari)


def visualizeData(Z, labels, settings, title):
    '''
    ...
    '''
    num_clusters = settings['num_class']
...
def sort_emb(emb, label):
    ...
    d = np.c_[emb, label_list.T]
    sort_emb = d[np.lexsort(d.T)]
    sort_emb = np.delete(sort_emb, -1, axis=1)
    # sort_emb = data_process.norm(sort_emb)
    cos_emb = cosine_similarity(sort_emb)

    return sort_emb, cos_emb    
    
def visualizeHeatmap(emb, label, title):
    _, cos_emb = sort_emb(emb, label)
    fig = plt.figure()
    heatmap(cos_emb, cmap='YlGnBu')
    fig.savefig(title, dpi=fig.dpi)
